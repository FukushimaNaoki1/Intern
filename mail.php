...
$site_top = "https://www.daiko-printing.co.jp/";

//管理者のメールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
//$to = "yamashita@daiko-printing.co.jp,kawamoto@daiko-printing.co.jp";
$to = "fukushima.naoki01@gmail.com";
//自動返信メールの送信元メールアドレス
//必ず実在するメールアドレスでかつ出来る限り設置先サイトのドメインと同じドメインのメールアドレスとすることを強く推奨します
//$from = "yamashita@daiko-printing.co.jp";
$from = "fukushima.naoki01@gmail.com";
...
$Referer_check_domain = "daiko-printing.co.jp";
...
$refrom_name = "大興印刷株式会社";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "【大興印刷】お問い合わせありがとうございました。";
...
お問い合わせありがとうございました。
内容を確認後担当者よりご連絡させていただきます。
お問い合わせ内容によっては、お返事にお時間を頂戴する場合もございますので、あしからずご了承ください。
...
──────────────────────
大興印刷株式会社　大興太郎
〒540-0028 大阪府大阪市中央区常盤町1-2-13
TEL：06-4794-0086　FAX：06-4794-0087
E-mail:xxxx@daiko-printing.co.jp
URL: https://www.daiko-printing.co.jp/

認証取得
［2019.11］プライバシーマーク認証取得
［2019.02］環境推進工場登録
［2007.02］FSC(R)認証制度を取得
──────────────────────
...
if (isset($_FILES['upload_file1'])) {
	$upload_file1 = $_FILES['upload_file1'];
	$type1 = explode('.', $upload_file1["name"]);
	$img_name1 = $type1[0] . '_' . $time . '.' . $type1[count($type1) - 1];
	move_uploaded_file($upload_file1['tmp_name'], './tmp/' . $img_name1);
} else {
	$upload_file1["name"] = "";
	$img_name1="";
}
...
if (isset($_FILES['upload_file2'])) {
	$upload_file2 = $_FILES['upload_file2'];
	$type2 = explode('.', $upload_file2['name']);
	$img_name2 = $type2[0] . '_' . $time . '.' . $type2[count($type2) - 1];
	move_uploaded_file($upload_file2['tmp_name'], './tmp/' . $img_name2);
} else {
	$upload_file2["name"] = "";
	$img_name2="";
}
...
if (($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1) {
	...
	//差出人に届くメールをセット
	if ($remail == 1) {
		//$filename = "./img/h_logo.png";
		$imagename = "h_logo.png";
	...
	if ($use_envelope == 0) {
		if ($_POST["file1"] !== "") {
			//$path1 = "./tmp/". $_POST["file1"]."_".;
			//$type1 = explode('.', $_POST["file1"]);
			//$img_name1 = $type1[0] . '_' . $_POST["date"] . '.' . $type1[count($type1) - 1];
			$path3 = "./tmp/" . $_POST["change1"];
			$path4 = "./upload_file/" . $_POST["change1"];

			//rename($path1, $path3);
			rename($path3, $path4);
		}
		if ($_POST["file2"] !== "") {
			//$path2 = "./tmp/" . $_POST["file2"];
			//$type2 = explode('.', $_POST["file2"]);
			//$img_name2 = $type2[0] . '_' . $_POST["date"] . '.' . $type2[count($type2) - 1];

			$path5 = "./tmp/" . $_POST["change2"];
			$path6 = "./upload_file/" . $_POST["change2"];
...
?>
	...
	<body>
		...
		<header>
			<h1>WEBサービス セルフブランディング</h1>
			<p>谷町じぶんカード</p>
			<p>じぶんを自由に表現しませんか？</p>
			<nav>
				<ul>
					<li><a href="index.html">ホーム</a></li>
					<li><a href="index.html#line">ラインナップ</a></li>
				</ul>
			</nav>
		</header>
		...
		<section>
			<?php if ($empty_flag == 1) { ?>
				<div id="confirm">
					<h5>入力に誤りがあります。<br />下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h5>
					<div class="panel"><?php echo $errm; ?></div>
					<div align="center"><input type="button" value=" 前画面に戻る " onClick="history.back()" id="back"></div>
				</div>
			<?php } else { ?>
				<div id="confirm">
					<h3>確認画面</h3>
					...
					<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST" enctype="multipart/form-data">
						...
						<table>
							...
							<tr>
								...
								<td>
									<?php echo $_POST["email"]; //入力内容を表示
									?>
								</td>
							</tr>

							<tr>
								<th><label for="company-name">電話番号</label></th>
								<td>
									<?php
									if (isset($_POST["tel"])) {
										echo $_POST["tel"];
							...
							<tr>
								...
								<td>
									<?php echo nl2br($_POST["comment"]); //入力内容を表示
									?>
								</td>
							</tr>

							<tr>
								<th><label for="comment">アップロードファイル</label></th>
								<td>
									<?php
									if ($upload_file1['error'] == 2) {
										echo "ファイルが大きいのでアップできません。<br>30MB以下でアップしてください。";
									} else {
										echo $upload_file1['name'];
									}
									?>
								</td>
							</tr>
							<tr>
								...
								<td>
									...
									<?php if ($upload_file2['error'] == 2) {
										echo "ファイルが大きいのでアップできません。<br>30MB以下でアップしてください。";
									} else {
										echo $upload_file2['name'];
									}
									?>
								</td>
							</tr>
						...
						<table hidden>
							<?php 
								echo confirmOutput($_POST, $upload_file1['name'], $upload_file2['name'], $img_name1, $img_name2);
							?>
						</table>

						<input type="hidden" name="mail_set" value="confirm_submit">
						<input type="hidden" name="httpReferer" value="<?php echo h($_SERVER['HTTP_REFERER']); ?>">
						<input type="button" value="前画面に戻る" onClick="history.back()" id="back">
		...
		<footer>
			<p id="logo"><img src="./img/h_logo.png" /><br />大興印刷株式会社<br /><a href="https://www.daiko-printing.co.jp/">https://www.daiko-printing.co.jp/</a></p>
			<div class="flex">
				<p>【本社】
					<br />大阪府大阪市中央区常盤町1−2−13
					<br />TEL：06-4794-0086
					<br />FAX：06-4794-0087
				</p>

				<p>【神戸ポートアイランド工場】
					<br />兵庫県神戸市中央区港島南町4−6−3
					<br />TEL：078-303-3660
					<br />FAX：078-303-3669
				</p>

				<p>【東京事務所】
					<br />東京都港区芝5-9-12　金沢ビル3階
					<br />TEL：03-6453-8639
					<br />FAX：03-6453-8638
				</p>
			</div>
			<p>&copy; 2021 daiko printing Inc. All Rights Reserved.</p>
		</footer>
		<!-- ▲ *********** 送信内容確認部　※編集は自己責任で ************ ▲-->
		...
		<?php if ($empty_flag == 1) { ?>
			<section>
				<div id="mailform" class="mailform">
					<h5>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h5>
...
<?php } else { ?>
	<section>
		<div id="confirm">
			<h3>完了画面</h3>
			<p class="panel">お問い合わせありがとうございました。
				<br />内容を確認後担当者よりご連絡させていただきます。
				<br />お問い合わせ内容によっては、お返事にお時間を頂戴する場合もございますので、あしからずご了承ください。
				<!-- <a href="<?php echo $site_top; ?>">トップページへ戻る&raquo;</a>-->
			</p>
			<p align="center"><a href="./index.html" id="back">ページに戻る</a></p>
	</section>
	<footer>
		<p id="logo"><img src="./img/h_logo.png" /><br />大興印刷株式会社<br /><a href="https://www.daiko-printing.co.jp/">https://www.daiko-printing.co.jp/</a></p>
		<div class="flex">
...
<?php
	...
	//確認画面の入力内容出力用関数
	function confirmOutput($arr, $files, $file, $file1, $file2)
	{
		...
		$html .= '<input type="hidden" name="file1" value="' . $files . '" />';
		$html .= '<input type="hidden" name="change1" value="' . $file1 . '" />';
		$html .= '<input type="hidden" name="file2" value="' . $file . '" />';
		$html .= '<input type="hidden" name="change2" value="' . $file2 . '" />';
		//トークンをセット
		if ($useToken == 1 && $confirmDsp == 1) {
	...
	function mailToAdmin($arr, $subject, $mailFooterDsp, $mailSignature, $encode, $confirmDsp)
	{
		...
		$adminBody .= "お名前：".$_POST["name"]."\n";
		$adminBody .= "ふりがな：".$_POST["hname"]."\n";
		$adminBody .= "会社名：".$_POST["company-name"]."\n";
		$adminBody .= "メールアドレス：".$_POST["email"]."\n";
		$adminBody .= "電話番号：".$_POST["tel"]."\n";
		$adminBody .= "お問い合わせ内容：".$_POST["comment"]."\n";
		$adminBody .= "ファイル1：".$_POST["file1"]."\n";
		$adminBody .= "ファイル2：".$_POST["file2"]."\n";
		$adminBody .= "\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n";
	...
	function mailToUser($arr, $dsp_name, $remail_text, $mailFooterDsp, $mailSignature, $encode)
	{
		...
		$userBody .= "お名前：".$_POST["name"]."\n";
		$userBody .= "ふりがな：".$_POST["hname"]."\n";
		$userBody .= "会社名：".$_POST["company-name"]."\n";
		$userBody .= "メールアドレス：".$_POST["email"]."\n";
		$userBody .= "電話番号：".$_POST["tel"]."\n";
		$userBody .= "お問い合わせ内容：".$_POST["comment"]."\n";
		$userBody .= "ファイル1：".$_POST["file1"]."\n";
		$userBody .= "ファイル2：".$_POST["file2"]."\n";
		$userBody .= "\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n\n";
		$userBody .= "送信日時：" . date("Y/m/d (D) H:i:s", time()) . "\n";
		//$userBody .= "--__PHPRECIPE__\r\n";
